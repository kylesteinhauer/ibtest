...
def plot_spread(
    ...
    if show_bands:
        ...
        ax.axhline(
            y=upper_band, color="orange", linestyle=":", alpha=0.7, label=f"+{n_std}σ"
        )
        ax.axhline(
            y=lower_band, color="orange", linestyle=":", alpha=0.7, label=f"-{n_std}σ"
        )

        # Fill between bands
        ax.fill_between(spread.index, lower_band, upper_band, alpha=0.1, color="orange")

    # Formatting
    pair_name = f"{result.symbol1} - {result.symbol2}"
    ax.set_title(
        f"Spread Analysis: {pair_name}\n"
    ...
    for i, result in enumerate(pairs_to_plot):
        ...
        if symbol1 in price_data.columns and symbol2 in price_data.columns:
            ...
            ax.plot(
                ...
                normalized_data[symbol1],
                label=symbol1,
                linewidth=1.5,
            )
            ax.plot(
                normalized_data.index,
                normalized_data[symbol2],
                label=symbol2,
                linewidth=1.5,
            )
...
def save_all_plots(
    ...
    fig1.savefig(
        f"{output_dir}/time_series.{file_format}", dpi=300, bbox_inches="tight"
    )
    plt.close(fig1)
    ...
    fig2.savefig(
        f"{output_dir}/correlation_heatmap.{file_format}", dpi=300, bbox_inches="tight"
    )
    plt.close(fig2)
    ...
    fig3.savefig(
        f"{output_dir}/summary_statistics.{file_format}", dpi=300, bbox_inches="tight"
    )
    plt.close(fig3)
    ...
    if cointegrated:
        ...
        fig4.savefig(
            f"{output_dir}/cointegrated_pairs.{file_format}",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig4)
        ...
        for _i, result in enumerate(cointegrated[:5]):  # Top 5 pairs
            fig = plot_spread(result)
            pair_name = f"{result.symbol1}_{result.symbol2}"
            fig.savefig(
                f"{output_dir}/spread_{pair_name}.{file_format}",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
